...
import styles from '../styles/PasswordEditor.module.scss';
...
const PasswordEditor = ({ password: initialPassword, onClose, onUpdateRequest, onPasswordSelect }: Props) => {
  ...
  const handleDeleteField = (id: string) => {
    setPassword((prev) => {
      let prevState: PasswordType = JSON.parse(JSON.stringify(prev));
  ...
  const savePassword = async () => {
    ...
    try {
      await api.update(password._id, { name: password.name, credentials: password.credentials, website: password.website });
      onUpdateRequest();
    } finally {
      setEditing(false);
      setLoading(false);
  ...
  return (
    <div className={styles.container}>
      ...
      <Modal onCloseRequest={() => setShowSelectIntegration(false)} show={showSelectIntegration} title='Select integration'>
        <SearchVault
          query={query}
